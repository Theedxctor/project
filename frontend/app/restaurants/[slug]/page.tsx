        ...
        <div style={{ display:'grid', gridTemplateColumns:'repeat(auto-fill,minmax(260px,1fr))', gap:'1.5rem' }}>
          {filtered.map(food => (
            <div key={food.id} style={{ background:'#13131a', border:'1px solid #2a2a3a', borderRadius:18, overflow:'hidden', transition:'transform 0.2s', opacity: food.is_available ? 1 : 0.5 }}>
              <div style={{ height:130, background:'linear-gradient(135deg,#1c1c27,#2a2a3a)', display:'flex', alignItems:'center', justifyContent:'center', fontSize:'2.5rem' }}>
                {food.name.toLowerCase().includes('ugali') ? '🍚' : food.name.toLowerCase().includes('chai') || food.name.toLowerCase().includes('tea') ? '🍵' : food.name.toLowerCase().includes('coffee') || food.name.toLowerCase().includes('espresso') || food.name.toLowerCase().includes('cappuccino') ? '☕' : food.name.toLowerCase().includes('burger') ? '🍔' : food.name.toLowerCase().includes('chips') ? '🍟' : food.name.toLowerCase().includes('chapati') ? '🫓' : food.name.toLowerCase().includes('juice') ? '🥤' : food.name.toLowerCase().includes('mandazi') ? '🍩' : food.name.toLowerCase().includes('pilau') ? '🍛' : food.name.toLowerCase().includes('sandwich') ? '🥪' : food.name.toLowerCase().includes('brownie') ? '🍫' : food.name.toLowerCase().includes('soda') ? '🥤' : food.name.toLowerCase().includes('water') ? '💧' : food.name.toLowerCase().includes('wings') ? '🍗' : '🍽'}
              </div>
